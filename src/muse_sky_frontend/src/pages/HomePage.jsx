...
function HomePage() {
  ...
  // Handle critical loading states
  if (isBtcLoading) {
  ...
  if (btcError) {
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center text-white px-4">
        <h1 className="text-2xl font-bricolage mb-4">
          Unable to load price data
        </h1>
        <p className="text-[#dfdfd1] mb-6">Please try refreshing the page</p>
        <CoolButton onClick={() => window.location.reload()}>
          Refresh Page
        </CoolButton>
      </div>
    );
  ...
  const renderBlogSection = () => {
    ...
    if (blogError) {
      return (
        <div className="text-center py-20">
          <p className="text-[#dfdfd1] mb-4">Unable to load blog posts</p>
          <CoolButton onClick={() => window.location.reload()}>
            Try Again
          </CoolButton>
        </div>
    ...
    // Check if blog posts exist and have content
    if (!blogPosts || blogPosts.length === 0) {
      return (
        <div className="text-center py-20">
          <p className="text-[#dfdfd1] mb-4">No blog posts available</p>
            ...
            <div className="flex-col justify-start items-start gap-6 flex">
              <div className="self-stretch flex-col justify-start items-start gap-2 flex">
                <div className="w-full md:w-[400px]">
                  <span className="text-white text-4xl md:text-5xl font-extrabold font-bricolage leading-tight">
                    Discover and collect Extraordinary{" "}
                  </span>
                  <span className="text-[#ffc252] text-4xl md:text-5xl font-extrabold font-bricolage leading-tight">
                    Weather
                  </span>
                  <span className="text-white text-4xl md:text-5xl font-extrabold font-bricolage leading-tight">
                    {" "}
                ...
                <div className="self-stretch text-white text-lg md:text-base font-normal font-onest mt-6">
                  Where Bitcoin meets digital art, evolving with nature's rhythm
                </div>
              </div>
            </div>
            <CoolButton onClick={() => navigate(ROUTES.COLLECTION)}>
              Collect Now
            </CoolButton>
          </div>
      ...
      <div className="bg-transparent relative">
        ...
        <div className="max-w-[100vw] py-[100px] overflow-hidden relative px-6 md:px-[100px]">
          <div className="mx-auto w-full flex-col justify-center items-center gap-6 inline-flex">
            <div className="text-white text-xl md:text-2xl font-normal font-onest text-center">
              Explore our new NFT collection
            </div>
            <div className="flex items-center gap-4">
              <div className="w-5 h-5 md:w-6 md:h-6 rotate-45 bg-white/20 rounded-md border border-white backdrop-blur-sm" />
              <h2 className="text-center text-white text-4xl md:text-5xl font-bold font-bricolage mx-2">
                Top NFTs
              </h2>
              <div className="w-5 h-5 md:w-6 md:h-6 rotate-45 bg-white/20 rounded-md border border-white backdrop-blur-sm" />
            </div>
        ...
        <div className="w-full relative py-[75px] md:py-[100px] overflow-hidden px-6 md:px-[100px]">
          <div className="max-w-6xl mx-auto">
            ...
            <CollectionCardCarousel
              collections={collections}
              className="mt-[75px] md:mt-[100px] max-w-5xl mx-auto"
            />
        ...
        <div className="max-w-[100vw] px-6 py-[75px] md:py-[100px] relative overflow-hidden">
          <div className="max-w-6xl mx-auto">{renderBlogSection()}</div>
