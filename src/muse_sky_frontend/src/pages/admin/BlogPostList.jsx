...
import { HiOutlinePencil, HiOutlineTrash, HiOutlineEye, HiChevronLeft, HiChevronRight } from 'react-icons/hi';
...
const BlogPostList = () => {
  ...
  useEffect(() => {
    const filtered = blogPosts.filter((post) => {
      ...
      const matchesDate = filters.date === 'all' || filterByDate(post.date, filters.date);
        ...
        <div className="bg-white shadow overflow-hidden sm:rounded-lg">
          ...
          <ul className="divide-y divide-gray-200">
            {currentPosts.map((post) => (
              <li key={post.id} className="px-4 py-4 sm:px-6 hover:bg-gray-50 transition duration-150 ease-in-out">
                <div className="flex items-center justify-between">
                  <div className="flex-1 min-w-0 pr-4">
                    <p className="text-lg font-semibold text-gray-900 truncate">{post.title}</p>
                    <p className="mt-1 flex items-center text-sm text-gray-500">
                      <span>{new Date(post.date).toLocaleDateString()}</span>
                      <span className="mx-2">&middot;</span>
                      <span className={`px-2 py-1 inline-flex text-xs leading-4 font-medium rounded-full ${post.status === 'published' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
                        }`}>
                        {post.status}
                      </span>
                    </p>
