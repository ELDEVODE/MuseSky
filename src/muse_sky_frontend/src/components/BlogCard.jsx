...
const BlogCard = ({ id, title, content, imageUrl, date }) => {
  ...
  return (
    ...
    >
      <img className="w-full h-48 rounded-lg object-cover" src={imageUrl} alt={title} />
      <div className="py-4 px-0 flex flex-col flex-grow">
        <div className="flex justify-between items-start gap-2">
          <h2 className="text-white text-lg font-semibold font-['Onest'] leading-tight mb-2 line-clamp-2">{title}</h2>
        ...
        <p className="text-[#dfdfd1] text-sm font-normal font-['Onest'] leading-normal mb-3 flex-grow line-clamp-3">{content}</p>
        <div className="text-[#8c8c8c] text-sm font-normal font-['Onest']">{date}</div>
      </div>
    </div>
  );
};
